...
# 2) user-selected class, use every piece in the class (cycle through), use a different color each placement
# 3) user-selected class, use every piece in the class (cycle through), use a random color each placement
# 4) user-selected class, use every piece in the class (cycle through), use same color each placement
...
def main():
    ...
    grid_n = ask_int("Board size (N for N x N, 8..16)", 8, 20, DEFAULT_GRID_COLS)
    poly_choice_token = ask_choice("Which polyominoes to use", ["tri","tet","pen","mix"], "pen")
    ...
    if placement_choice == "random":
        ...
        if not pool:
            ...
            if occupied_squares >= target_squares:
                print(f"Random placement complete. Placed {placed_count} pieces, occupied squares: {occupied_squares}/{target_squares}")
            else:
    ...
    while running:
        ...
        if show_instructions:
            ...
            #    "Right click to rotate 90° CW.",
            #    "Middle click to flip horizontally.",
