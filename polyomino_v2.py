...
def main():
    ...
    while True:
        ...
        raw_color = ask_choice("Color choice", ["unique","random","same","u","r","s"], "random")
        # normalize
        ...
        for i, (name, cells) in enumerate(chosen):
            # assign a default color for listing consistent with policy
            if color_choice == "unique":
                color = unique_color_map.get(name, random.choice(PALETTE))
            elif color_choice == "same":
                color = shared_color
            else:
