...
# to choose:
#  - square board size (N x N), 8 <= N <= 20
#  - degree of difficulty: integer 1..6 (controls selection & coloring rules)
#  - which polyomino class to use (tri/tet/pen/mix) [only shown for degrees 1..3]
#
# Degree mapping (1..6) controls which pieces are selected and color policy:
# 1) user-selected class, cycle through class, use a different color each placement
# 2) user-selected class, cycle through class, use a random color each placement
# 3) user-selected class, cycle through class, use same color each placement
# 4) mixed classes, randomly choose any piece, use a different color each placement
# 5) mixed classes, randomly choose any piece, use a random color each placement
# 6) mixed classes, randomly choose any piece, use same color each placement
#
...
def main():
    ...
    while True:
        ...
        degree = ask_int("Degree of difficulty (1..6)", 1, 6, 4)

        def degree_uses_class_only(deg):
            # After renumbering, degrees 1..3 use class-only behavior
            return deg in (1, 2, 3)
        ...
        # For degrees 1-3, use the user-selected class; for 4-6, use mixed (all)
        if degree_uses_class_only(degree):
        ...
        # For "use every piece" degrees: formerly (2,3,4) -> now (1,2,3)
        use_every_piece = degree in (1, 2, 3)

        # Color policies mapping after renumbering:
        # new degrees:
        # 1 -> different, 2 -> random, 3 -> same, 4 -> different, 5 -> random, 6 -> same
        if degree in (1, 4):
        ...
        # If 'Repeat' was requested, clean up and loop back to the outer while True to re-run prompts & placement
